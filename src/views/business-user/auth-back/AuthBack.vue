    ...
    <div>
        ...
        <div class="content" :style="status.back ? 'padding-top: 0;' : ''">
            ...
            <div v-show="status.submit">
                ...
                <div class="title flex-center" style="margin-top: 60px;">
                    已接收您的退标数量为：<span
                        style="font-weight: bold;font-size: 60px;margin-right: 10px;"
            ...
            <div v-show="status.success">
                ...
                <div class="title flex-center" style="margin-top: 60px;">
                    您已成功退标成功：<span
                        style="font-weight: bold;font-size: 36px;margin-right: 10px;"
                        >{{ returnNum }}</span
                    >&nbsp;枚
                </div>
        ...
        <div style="text-align: center;">
            ...
            <AioBtn v-show="status.warn" @click="statusTransfer('back')"
                >我已知晓</AioBtn
            >
            <AioBtn v-show="status.back" :loading="loading" @click="doBack"
                >开始退标</AioBtn
            >
            <!-- todo 用户忘记点结束退标，需自动提交：用户取走茶农卡时，如果在退标页且未提交 -->
            <AioBtn
                v-show="status.submit"
                :disabled="$store.getters.noReturn"
                :loading="loading"
                @click="doSubmit"
                >完成退标</AioBtn
            >
            <AioBtn v-show="status.success" @click="reBack">再退一笔</AioBtn>
...
export default {
    ...
    methods: {
        ...
        /** helpers */
        // 保证退标设备正常 todo 放到路由去做
        readyReturnBox() {
            this.$store.dispatch('readyReturnBox')
        },
